...
function getFlashDownloadUrl() {
	if (navigator.userAgent.match(/.msie./ig)) {
		return '../statics/flash/install_flash_player_ax.exe';
	} else {
		return '../statics/flash/install_flash_player.exe';
	}
}
$("#download_swf a").attr('href',getFlashDownloadUrl());
...
var TEQU = TEQU || {};
TEQU.namespace = function(_space) {
	var parts = _space.split('.'),
		parent = TEQU;
	//剥离前面的冗余变量
	if (parts[0] === "TEQU") {
		parts = parts.slice(1);
...
TEQU.Plugin = function(size, option) {
	var Ansy = TEQU.Plugin.Ansy;
	var Method=TEQU.Plugin.Method;
	var	Event=TEQU.Plugin.Event;
	this.size = size;
	this.option = option;
	Ansy.init();
	Method.init(size, option);
	Event.init();
	Method.loadCSS("http://www.tetequ.com/statics/css/pluins.css");
	return this;
};
/**
	事件绑定
**/
TEQU.Plugin.Event=(function(){
	return{
		init:function(){
			$('#tetequ').on("click","a",function(){
				...
				TEQU.Plugin.Method.ckeckIn($(this));
			});
		}
	}
})();
/**
	方法实现
**/
TEQU.Plugin.Method=(function(){
	return{
		option:{},
		size:{},
		init:function(size,option){
			this.option=option;
			this.size=size;
			this.createLoading();
			this.createDom();
		...
		**/
		checkKey:function(){
			...
			var encode=TEQU.Plugin.Untils.encode;
			return encode.hex_md5(location.host+option.uid).substr(8,16)==option.key?true:false;
		},
		createLoading:function(){
			$("#tetequ").css(this.size);
		...
		createDom:function(){
			...
			if(!this.checkKey()) {
				...
				var create = function(data) {
					$("#tetequ").html(_this.createInput(data));
					$("#tetequ, #tetequ img").css(_this.size);
				};
				var URL="http://www.tetequ.com/embed/get_room_info?roomid="+_this.option.uid;
				_this.IEXdomain(URL,create);
			}
		},
		//解决IE跨域问题
		IEXdomain: function(url,fn) {
			if (TEQU.Plugin.Ansy.info.browser.type != "IE"|| (TEQU.Plugin.Ansy.info.browser.version>10&&TEQU.Plugin.Ansy.info.browser.type == "IE")) {
				$.ajax({
		...
		createSWF:function(data){
			$('body').empty().html("<div id='website'></div>").css({"height":"100%"});
			swfobject.embedSWF(data.main_framework, "website", "100%", "100%",
				swfVersionStr, "http://www.tetequ.com/statics/flash/expressInstall.swf",
				data.flashvars, params, attributes);
			var swfObj;
			window.onbeforeunload = function() {
				swfObj = swfobject.getObjectById("main");
				return "你确定要离开吗？";
			}
		...
		ckeckIn:function(obj){
			var _this=this;
			var URL="http://www.tetequ.com/embed/get_live_param?roomid="+this.option.uid;
			var nick=obj.parent().find('.tetequ-nick input').val()?obj.parent().find('.tetequ-nick input').val():(TEQU.Plugin.Ansy.info.city+"网友"+Math.floor(Math.random()*10000));
			...
			var check=function(data){
				if(data.full){
					$(".tetequ-nick").tooltip({"title":"该房间人数已满，请稍后进入","placement":"top","trigger":""}).tooltip("show");
					return false;
				}
				if(data.password){
					$(".tetequ-pwd").tooltip({"title":"您输入的密码错误","placement":"top","trigger":""}).tooltip("show");
				}else{
					_this.createSWF(data);
				}
			}
			this.IEXdomain(URL,check);
		},
		loadCSS:function(fileurl){
 ...
 *	分析插件
 */
TEQU.namespace("TEQU.Plugin.Ansy");
TEQU.Plugin.Ansy = (function() {
	return {
		info: {},
		init: function() {
			this.referrer();
		...
		checkBrowser: function() {
			...
			var s;
			(s = ua.match(/(msie\s|trident.*rv:)([\w.]+)/)) ? (Sys.type = 'IE', Sys.version = s[2]) :
			...
			this.info.browser = Sys;
		},
		ipCheck: function() {
			var _this = this;
			var URL="http://www.tetequ.com/embed/get_ip_info";
			var upMsg=function(data){
				_this.info.ip = data.ip;
				_this.info.city = data.city;
				_this.info.isp = data.isp;
				_this.upMsg();
			}
			TEQU.Plugin.Method.IEXdomain(URL,upMsg);
		},
		upMsg: function() {
			var URL="http://www.tetequ.com/embed/user_data?ip="+this.info.ip+"&city="+this.info.city+"&isp="+this.info.isp+"&referrer="+this.info.referrer+"&type="+this.info.browser.type+"&version="+this.info.browser.version+"&userAgent="+navigator.userAgent+"&roomid="+TEQU.Plugin.Method.option.uid;
			TEQU.Plugin.Method.IEXdomain(URL,function(){;});
		}
	}
})();
TEQU.namespace("TEQU.Plugin.Untils");
TEQU.Plugin.Untils.encode = (function() {
	return {
		hexcase: 1,
		/* hex 输出格式. 0 - 小写; 1 - 大写       */
		b64pad: "",
